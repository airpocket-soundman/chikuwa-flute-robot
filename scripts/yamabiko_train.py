...
and the first-step onset error of the policy with memory carried, the same
...
def report(gen: int, ev: dict) -> None:
    for name, r in ev.items():
        cells = "  ".join(f"{d['mean_abs']:6.1f}/{d['onset1']:5.1f}" for d in r["songs"])
        print(f"  gen {gen:4d} {name:12s} score {r['score']:+.4f}  |cents|/onset1 per song: {cells}", flush=True)
...
def main() -> None:
    ...
    args = ap.parse_args()
    if args.pop % 2 or args.pop < args.workers:
    ...
    spec = TaskSpec(rigs=args.rigs, songs=args.songs, hidden=args.hidden, harsh=args.harsh, spread=args.spread,
                    swap_prob=args.swap_prob, weight_growth=args.weight_growth)
    rng = np.random.default_rng(args.seed)
    theta = GRUPolicy.init_params(rng, args.hidden)
    if args.init:
        d = np.load(args.init)
    ...
    print(f"params {theta.size}  pop {args.pop}  rigs {args.rigs}  songs {args.songs}  workers {args.workers}", flush=True)

    eval_seed = 10_000_019
    best = -np.inf
    with Pool(args.workers, initializer=_init, initargs=(bank_file,)) as pool:
        half = args.pop // 2
        for g in range(1, args.gens + 1):
            ...
            f = np.concatenate(pool.map(_fitness, [(thetas[c], spec, seed) for c in chunks]))
