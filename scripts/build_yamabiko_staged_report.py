...
def main():
    ...
    hybrid_lab_html = f'''<section class="hybrid-lab" id="hybrid-lab">
      ...
      <fieldset class="hybrid-series" aria-label="グラフに表示する工程">
        <legend>グラフ表示（各工程を個別に比較）</legend>
        <label><input type="checkbox" data-hybrid-series="reference" checked><i class="reference"></i>お手本</label>
        <label><input type="checkbox" data-hybrid-series="ear"><i class="ear"></i>1 聴覚</label>
        <label><input type="checkbox" data-hybrid-series="memory"><i class="memory"></i>2 記憶</label>
        <label><input type="checkbox" data-hybrid-series="planner" checked><i class="planner"></i>3 計画</label>
        <label><input type="checkbox" data-hybrid-series="performance" checked><i class="performance"></i>4 最終演奏</label>
      </fieldset>
      <canvas class="hybrid-chart" role="img" aria-label="お手本とチェックした各工程の時間対音程グラフ"></canvas>
...
<div class="verdict"><b>最新の計画と結果は <a href="yamabiko-plan.html">やまびこ1号 開発計画と結果</a> にまとめています。</b> このページは、そこに至るまでの試行の記録です。</div>
...
(() => {{
  ...
  const seriesChecks = Object.fromEntries([...root.querySelectorAll('[data-hybrid-series]')]
  ...
  function alignTrack(track, length) {{
    if (!track || !track.pitch_cents || !track.pitch_cents.length) return {{pitch_cents:[], voice:[]}};
    if (track.pitch_cents.length === length) return track;
    const ratio = length / track.pitch_cents.length;
    const sourceIndex = index => Math.min(track.pitch_cents.length-1, Math.floor(index/ratio));
    return {{
      pitch_cents:Array.from({{length}}, (_,index) => track.pitch_cents[sourceIndex(index)]),
      voice:Array.from({{length}}, (_,index) => track.voice[sourceIndex(index)])
    }};
  }}

  function draw(entry, sample) {{
    ...
    const playbackLength = entry.memory.pitch_cents.length;
    ctx.clearRect(0,0,width,height); const m={{l:58,r:18,t:22,b:42}}, lo=600, hi=2000;
    const x = i => m.l + i*(width-m.l-m.r)/Math.max(1,playbackLength-1);
    ...
    ctx.fillText('音程 [cent]',8,15); ctx.fillText(`時間 [s]  0 — ${{(playbackLength/100).toFixed(1)}}`,m.l,height-12);
    const plot = (track,color,dash=[],lineWidth=2) => {{ctx.strokeStyle=color;ctx.lineWidth=lineWidth;ctx.setLineDash(dash);ctx.beginPath();let open=false;track.pitch_cents.forEach((v,i)=>{{if(!track.voice[i]){{open=false;return;}}const px=x(i),py=y(v);if(!open){{ctx.moveTo(px,py);open=true;}}else ctx.lineTo(px,py);}});ctx.stroke();ctx.setLineDash([]);}};
    const referenceTrack = alignTrack(sample.reference, playbackLength);
    const earTrack = alignTrack(entry.ear, playbackLength);
    const plannerTrack = {{pitch_cents:entry.planner.pitch_cents || entry.planner.nominal_cents,
                          voice:entry.planner.voice}};
    const series = {{
      reference:[referenceTrack,'#f2f5f7',[],3],
      ear:[earTrack,'#bd93f9',[],2],
      memory:[entry.memory,cyan,[],2],
      planner:[plannerTrack,green,[],2],
      performance:[entry.performance,orange,[6,4],2]
    }};
    Object.entries(series).forEach(([name,args]) => {{if(seriesChecks[name]?.checked) plot(...args);}});
  ...
  function render() {{
    ...
    updateAudio('planner', {{pitch_cents:entry.planner.pitch_cents || entry.planner.nominal_cents,
                            voice:entry.planner.voice}});
    updateAudio('control', entry.performance); draw(entry,sample);
  }}
  sampleSelect.addEventListener('change', render);
  stages.forEach(name => selects[name].addEventListener('change', render));
  Object.values(seriesChecks).forEach(input => input.addEventListener('change', () => current && draw(current.entry,current.sample)));
  root.querySelector('.hybrid-run').addEventListener('click', render);
  window.addEventListener('resize', () => current && draw(current.entry,current.sample)); render();
}})();
</script></body></html>'''
