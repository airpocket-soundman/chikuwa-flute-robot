...
# policies (default runs/yamabiko_gru.npz). Then on the UNO Q: cd /home/arduino/yamabiko && python3 scripts/...
set -e
cd "$(dirname "$0")/.."
...
for f in scripts/yamabiko_collect.py scripts/yamabiko_play.py scripts/yamabiko_link.py; do
  "$ADB" push "$f" "$D/$f" >/dev/null
done
