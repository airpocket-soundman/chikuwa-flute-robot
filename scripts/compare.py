...
def controllers(residual: str | None) -> dict:
    out = {
    ...
    if residual:
        net, meta = MLP.load(residual)
        out["adaptive + RL residual"] = lambda: ModelResidualPolicy(
            AdaptivePolicy(ilc_gain=float(meta["ilc_gain"])), net, scale=float(meta["scale"]), horizon=int(meta["horizon"]))
    return out


def _init(args):
    _A["args"] = args
    _A["ctrl"] = controllers(args.residual)


def _job(task):
    name, seed = task
    a = _A["args"]
    env = FluteEnv(progress=a.progress, takes=a.takes)
...
def main() -> None:
    ...
    names = list(controllers(args.residual))
