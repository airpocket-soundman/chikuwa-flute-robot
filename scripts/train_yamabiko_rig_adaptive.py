...
def episode_loss(model, plant, songs, params, generator, carry=True):
    memory, losses, maes = None, [], []
    for cents, voice in songs:
...
def evaluate(model, plant, songs, params):
    ...
    for label, carry in (("carry", True), ("reset", False)):
        _, maes = episode_loss(model, plant, songs, params, torch.Generator(params.torque_gain.device).manual_seed(0), carry)
...
def main():
    ...
    model = RigAdaptivePerformer(RigAdaptiveConfig()).to(device)
    ...
    for step in range(1, args.steps + 1):
        ...
        if (step % args.eval_every == 0 and step > args.warmup_steps) or step == args.steps:
            metrics = evaluate(model, plant, eval_songs, eval_params)
    ...
    report = {"best": best, "warmup_steps": args.warmup_steps, "lr": args.lr, "history": history, "seed": args.seed, "steps": args.steps,
