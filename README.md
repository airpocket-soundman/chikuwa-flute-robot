...
> 学習は `scripts/train_yamabiko_e2e.py`、実機推論は `scripts/yamabiko_e2e_play.py` です。
> 推論経路、学習済みシミュレーターチェックポイント、NumPy書き出し、テストを実装済みです。実機評価はまだありません。
> コンテストでは、**生音声の知覚 → 物理的な行動 → 実際に鳴った生音声 → 同じモデルの内部状態の適応**という
> 閉ループ全体をフィジカルAIとして示します。笛を交換して一度崩れ、同じ重みのまま吹くほど再適応するデモを中心にします。
> UNO QではPyTorchを使わず、64,206パラメーターのモデルを約245KiBのNumPy形式へ書き出します。
> 機能優先のPC基準モデルは812,822パラメーター（約2.94MiB）で、成功後に小型版へ蒸留します。
> MCUの16kHz PCMストリームから毎10ms、最新20msを直接入力し、YINは実行しません。
> マイク・室内音響も乱数化した未学習4 seed・各64条件（合計256条件）では、音程誤差が平均 **271.6 → 277.0 → 307.6セント**、
> 自己音を遮断すると **294.3 → 326.4 → 379.3セント**です。従来モデルの乱数化時平均387.9セントから285.4セントへ改善し、閉ループ利得も平均48.0セント再現しました。反復ごとの単調な上達はまだ未達です。
> モデル構造、強化学習、採否を含む評価は [HTMLレポート](docs/e2e-training-report.html) に図解しています。
> 公開版は [GitHub Pages の段階別学習レポート](https://airpocket-soundman.github.io/chikuwa-flute-robot/e2e-training-report.html) で、各Gateの前後WAVを再生できます。
